...
function CreateCourse() {
  ...
  // Deteta tentativa de mudança de página
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className="min-vh-100 d-flex align-items-center justify-content-center bg-light">
        <div className="card shadow-lg p-5 border-0 w-100" style={{ maxWidth: "850px", marginTop: "80px", marginBottom: "80px" }}>
          ...
          <div className="mb-4">
            ...
            {!previewUrl ? (
              <div className="border rounded p-4 text-center bg-light" style={{ cursor: "pointer" }}>
                ...
                <input type="file" id="upload" accept="image/*" onChange={handleImageSelect} style={{ display: "none" }} />
              </div>
            ) : (
              <div className="text-center">
                <img src={previewUrl} alt="preview" className="img-fluid rounded mb-2" style={{ maxHeight: "200px" }} />
                <br />
                <button type="button" className="btn btn-sm btn-outline-danger" onClick={handleRemoveImage}>Remover imagem</button>
              </div>
            )}
          </div>
          ...
          <div className="mb-4">
            ...
            <div className="d-flex gap-2">
              <input type="text" className="form-control" value={newSectionTitle} onChange={(e) => setNewSectionTitle(e.target.value)} placeholder="Título da secção" />
              <button className="btn btn-success" type="button" onClick={addSection}><FaPlus /></button>
            </div>
          </div>
          ...
          {sections.map((section, sectionIndex) => (
            <div key={sectionIndex} className="mb-4 border p-3 rounded bg-white position-relative">
              <h5 className="fw-bold d-flex justify-content-between align-items-center">
                ...
                <IoTrashOutline size={20} onClick={() => removeSection(sectionIndex)} style={{ cursor: "pointer" }} className="text-danger" />
              </h5>
              <button className="btn btn-sm btn-outline-success mb-3" type="button" onClick={() => addItemToSection(sectionIndex)}>+ Adicionar Item</button>
              {section.items.map((item, itemIndex) => (
                <div key={itemIndex} className="border rounded p-3 mb-3 position-relative">
                  <div className="d-flex justify-content-between align-items-center mb-2">
                    ...
                    <IoTrashOutline size={18} onClick={() => removeItemFromSection(sectionIndex, itemIndex)} style={{ cursor: "pointer" }} className="text-danger" />
                  </div>
                  <input type="text" className="form-control mb-2" value={item.title} onChange={(e) => updateItemField(sectionIndex, itemIndex, "title", e.target.value)} />
                  <label className="form-label">Vídeo (obrigatório)</label>
                  <input type="file" accept="video/*" className="form-control mb-2" onChange={(e) => updateItemField(sectionIndex, itemIndex, "video", e.target.files[0])} required />
                  <label className="form-label">Ficheiro (opcional)</label>
                  <input type="file" className="form-control" onChange={(e) => updateItemField(sectionIndex, itemIndex, "file", e.target.files[0])} />
                </div>
              ))}
            </div>
          ))}
          ...
          <div className="text-end">
            <button className="btn btn-dark px-4" onClick={handleFinalizar}>Finalizar</button>
        ...
        <div className="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center" style={{ backgroundColor: "rgba(0,0,0,0.5)", zIndex: 1050 }}>
          <div className="bg-white p-4 rounded shadow text-center" style={{ maxWidth: "400px" }}>
            ...
            <div className="d-flex justify-content-center gap-3">
              <button className="btn btn-danger" onClick={handleConfirmExit}>Sim, sair</button>
              <button className="btn btn-secondary" onClick={handleCancelNavigation}>Cancelar</button>
            </div>
            <button className="btn btn-sm position-absolute top-0 end-0 m-2" onClick={handleCancelNavigation}>
              <IoClose size={18} />
            </button>
          </div>
        </div>

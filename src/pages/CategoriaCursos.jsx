...
function CategoriaCursos() {
  ...
  useEffect(() => {
    document.title = `Categoria: ${nome} | EDUX`;
    fetchCursosPorCategoria();
    fetchCursosSubscritos();
  }, [nome]);

  const fetchCursosPorCategoria = async () => {
  ...
  return (
    <>
      ...
      <div className="container" style={{ marginTop: "100px", minHeight: "70vh" }}>
        <h2 className="mb-5 text-center">Cursos de {nome}</h2>

        {loading ? (
          <p className="text-center">A carregar cursos...</p>
        ) : cursos.length === 0 ? (
          <p className="text-center text-muted">Nenhum curso encontrado nesta categoria.</p>
        ) : (
          <div className="row">
            {cursos.map((curso) => {
